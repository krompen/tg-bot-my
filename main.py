...
async def do_search(callback: CallbackQuery):
    ...
    progress_msg = await callback.message.answer("🔄 Ищу свободный тег... (это может занять до 30 секунд)")
    ...
    try:
        ...
        await callback.message.answer("⏰ Поиск занял слишком много времени. Попробуй ещё раз или выбери другую длину.")
        return
...
async def main():
    ...
    await bot.set_my_commands([
        BotCommand(command="search", description="🔍 Поиск тега"),
        BotCommand(command="profile", description="👤 Профиль"),
        BotCommand(command="referrals", description="👥 Рефералы"),
        BotCommand(command="support", description="🛠 Поддержка"),
        BotCommand(command="status", description="📊 Возможности статусов"),
    ])
    
    await telethon_client.start()
    logger.info("ФИНАЛЬНЫЙ БОТ ЗАПУЩЕН (всё исправлено + быстрые команды + красивый поиск)")
