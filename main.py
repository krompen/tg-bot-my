...
async def is_username_available(username: str) -> bool:
    """
    Проверяет, можно ли реально занять юзернейм.
    Возвращает True только если юзернейм:
    - Не занят в Telegram
    - Не является NFT на Fragment
    - Не зарезервирован Telegram
    """
    # 1. Проверка через Telethon
    try:
        ...
        return False  # Ошибка — считаем занятым

    # 2. Проверка через t.me (самая точная)
    try:
        import aiohttp
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, timeout=8) as resp:
                ...
                if "Send Message" in text or "t.me/" + username in text.lower():
                    return False
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, timeout=6) as resp:
                ...
                if "This username is available" in text or "not for sale" in text.lower():
                    return False
                if "Buy now" in text or "Current bid" in text or " fragment.com" in text.lower():
                    return True
                return False
    except:
        return False
...
GOOD_PATTERNS = [
    ...
    # Дополнительные красивые комбинации
...
async def find_free_username(length: int, max_attempts: int = 250, user_id: int = None) -> tuple:
...
async def do_search(callback: CallbackQuery):
    ...
    progress_msg = await callback.message.answer("🔄 Ищу свободный тег... (до 60 секунд)")
    
    try:
        free_username, status = await asyncio.wait_for(
            find_free_username(length, user_id=callback.from_user.id),
            timeout=55.0
        )
    ...
    if free_username:
        ...
        text = (
            ...
            f"⭐ Редкость: {stars}\n"
            f"{status_emoji}\n\n"
            f"Его вы можете продать в нашем OTC\n@otcbond"
        )
...
async def promo_menu(callback: CallbackQuery):
    text = (
        "🎟 <b>Промокоды</b> 🎟\n\n"
        "Введите код промокода, чтобы активировать награду!\n\n"
        "Пример: TEQQ2026"
    )
    await callback.message.answer(text)
    await callback.answer()
...
async def handle_promo_activation(message: Message):
    ...
    if not re.match(r'^[A-Z0-9]+$', text):
        return
    if text.startswith('/') or text in ['START', 'SEARCH', 'PROFILE', 'REFERRALS', 'SUPPORT', 'STATUS']:
        return
    
    async with aiosqlite.connect(DB_NAME) as db:
        cursor = await db.execute("SELECT * FROM promo_codes WHERE code = ?", (text,))
        ...
        cursor = await db.execute("SELECT * FROM promo_activations WHERE code = ? AND user_id = ?", (text, message.from_user.id))
        if await cursor.fetchone():
            await message.answer("❌ Вы уже активировали этот промокод.")
            return
        
        await db.execute("UPDATE promo_codes SET uses_left = uses_left - 1 WHERE code = ?", (text,))
        await db.execute("INSERT INTO promo_activations (code, user_id) VALUES (?, ?)", (text, message.from_user.id))
        ...
        if row:
            ...
            try:
                await bot.send_message(user_id, 
                    ...
                    f"Агент поддержки уже работает над вашим вопросом.\n"
                    f"Ожидайте ответ в ближайшее время! 🙏")
            except:
                pass
...
async def main():
    ...
    logger.info("✅ ФИНАЛЬНЫЙ БОТ ЗАПУЩЕН (улучшенная проверка + строгие промокоды + нет меню)")
