...
async def is_fragment_nft(username: str) -> bool:
    """Проверяет, является ли юзернейм NFT на Fragment.com"""
    try:
        import aiohttp
        async with aiohttp.ClientSession() as session:
            ...
            async with session.get(url, timeout=6) as resp:
                ...
                # Если на странице есть "This username is available" или "Not for sale" — не NFT
                if "This username is available" in text or "not for sale" in text.lower():
                    return False
                # Если есть "Buy now" или "Current bid" — вероятно NFT
                if "Buy now" in text or "Current bid" in text or "for sale" in text.lower():
                    return True
                return False
    except:
        return False  # если не получилось — считаем не NFT
...
async def approve_join_request(update: ChatJoinRequest):
    try:
        ...
        logger.info(f"Заявка одобрена для {update.from_user.id}")
...
GOOD_PATTERNS = [
    ...
    ("teqqen", "teqqen"), ("teqqix", "teqqix"), ("teqqon", "teqqon"), ("teqqar", "teqqar"),
    ("teqqen", "teqqen"), ("teqqix", "teqqix"), ("teqqon", "teqqon"), ("teqqar", "teqqar"),
    ("teqqen", "teqqen"), ("teqqix", "teqqix"), ("teqqon", "teqqon"), ("teqqar", "teqqar"),
    
    # Много английских красивых комбинаций
...
async def find_free_username(length: int, max_attempts: int = 80, user_id: int = None) -> tuple:
    for _ in range(max_attempts):
        username = generate_nice_username(length)
        is_free = await safe_check_username(username)
        if is_free:
            # Проверка через Fragment (по возможности)
            is_nft = await is_fragment_nft(username)
            if is_nft:
                continue  # пропускаем NFT
...
async def main():
    ...
    logger.info("Финальный бот с максимальным количеством слов + Fragment check + авто-одобрение")
