...
# === ВАЖНО: задай эти переменные на bothost.ru ===
PHONE = os.getenv("PHONE")   # например: +79001234567
CODE = os.getenv("CODE")     # код из Telegram (только при первом запуске)
...
async def main():
    ...
    # Логин через PHONE + CODE
    if not os.path.exists(f"{SESSION_NAME}.session"):
        if not PHONE or not CODE:
            logger.error("Не заданы PHONE или CODE в переменных окружения!")
            return
        ...
        logger.info("Успешный вход в Telegram аккаунт")
    else:
        await telethon_client.start()
