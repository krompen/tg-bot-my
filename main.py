...
async def find_free_username(length: int, max_attempts: int = 60) -> tuple:
    for _ in range(max_attempts):
        username = generate_nice_username(length)
        is_free = await safe_check_username(username)
        if is_free:
            if len(set(username)) < 4 or username.count(username[0]) > 3:
                continue
...
async def support_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(
        "🛠 <b>Поддержка</b> 🛠\n\n"
        "Опиши свою проблему или вопрос. Мы ответим в ближайшее время.\n\n"
        "Напиши сообщение:"
    )
    await callback.answer()
...
async def support_save(message: Message, state: FSMContext):
    ...
    await message.answer("✅ Спасибо! Твой тикет создан. Мы ответим в ближайшее время.")
    await state.clear()
...
@dp.callback_query(F.data == "admin_reply_ticket")
async def admin_reply_ticket_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != OWNER_ID: return
    await state.set_state(AdminTicketReplyState.waiting_ticket_id)
    await callback.message.edit_text("🎟 Введите номер тикета для ответа:")

@dp.message(AdminTicketReplyState.waiting_ticket_id)
async def admin_reply_ticket_get(message: Message, state: FSMContext):
    if message.from_user.id != OWNER_ID: return
    try:
        ticket_id = int(message.text)
        await state.update_data(ticket_id=ticket_id)
        await state.set_state(AdminTicketReplyState.waiting_reply_text)
        await message.answer("✍️ Напиши ответ пользователю:")
    except:
        await message.answer("❌ Неверный номер тикета.")

@dp.message(AdminTicketReplyState.waiting_reply_text)
...
async def buy_premium(callback: CallbackQuery):
    ...
    prices = {1: 25, 3: 65, 7: 140, 30: 450}
    amount = prices.get(days, 25)
    
    await bot.send_invoice(
        chat_id=callback.from_user.id,
        title=f"Премиум на {days} дней",
        description=f"Получи неограниченные поиски и приоритет на {days} дней",
        payload=f"premium_{days}",
        provider_token="",  # Для Stars оставь пустым
        currency="XTR",
        prices=[LabeledPrice(label=f"Премиум {days} дней", amount=amount)],
...
async def successful_payment(message: Message):
    ...
    if payload.startswith("premium_"):
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute(
                "UPDATE users SET is_premium = 1, premium_until = ?, role = 'premium' WHERE user_id = ?",
                (until, message.from_user.id)
            )
            await db.commit()
        
        await message.answer(f"✅ Премиум активирован на {days} дней! Спасибо за покупку!")

# ==================== АДМИН: УСТАНОВКА ФОТО ====================
...
async def admin_set_photo_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("🖼 Отправь фото, которое будет использоваться как приветствие:")
...
async def main():
    ...
    logger.info("Полный рабочий бот запущен")
