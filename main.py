...
async def is_username_available(username: str) -> bool:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(f"https://t.me/{username}", timeout=7) as resp:
                if resp.status != 200:
                    return False
...
async def find_free_username(length: int, max_attempts: int = 180, user_id: int = None) -> tuple:
...
async def do_search(callback: CallbackQuery):
    ...
    progress_msg = await callback.message.answer("🔄 Ищу свободный тег... (до 45 секунд)")
    
    try:
        free_username, status = await asyncio.wait_for(find_free_username(length, user_id=callback.from_user.id), timeout=42.0)
    ...
    if free_username:
        ...
        text = f"✅ <b>НИК НАЙДЕН!</b> ✅\n\n@{free_username}\n📏 {length} букв\n\n⭐ Редкость: {stars}\n{status_emoji}\n\nЕго вы можете продать в нашем OTC\n@otcbond"
...
async def main():
    ...
    # УБИРАЕМ ВСЕ КОМАНДЫ ИЗ МЕНЮ
    await bot.set_my_commands([])
    logger.info("✅ ФИНАЛЬНЫЙ БОТ ЗАПУЩЕН (промокоды только через FSM + нет меню команд)")
