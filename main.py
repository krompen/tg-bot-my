...
async def find_free_username(length: int, max_attempts: int = 150, user_id: int = None, user_status: str = "user") -> tuple:
...
async def do_search(callback: CallbackQuery):
    ...
    try:
        free_username, status = await asyncio.wait_for(
            find_free_username(length, user_id=callback.from_user.id, user_status=user_status),
            timeout=35.0
        )
...
async def handle_promo_activation(message: Message):
    ...
    if not (4 <= len(text) <= 20 and text.isalnum()):
        return
...
async def main():
    ...
    logger.info("ФИНАЛЬНЫЙ БОТ ЗАПУЩЕН (всё исправлено + промокоды + поиск + статусы)")
