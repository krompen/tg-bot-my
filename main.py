...
async def find_free_username(length: int, max_attempts: int = 120, user_id: int = None) -> tuple:
...
async def do_search(callback: CallbackQuery):
    ...
    progress_msg = await callback.message.answer("🔄 Ищу свободный тег... (до 35 секунд)")
    
    try:
        free_username, status = await asyncio.wait_for(find_free_username(length, user_id=callback.from_user.id), timeout=33.0)
...
async def admin_give_status_final(message: Message, state: FSMContext):
    ...
    actual_status = updated_user[4] if updated_user else "unknown"
    
    log_text = f"✅ <b>Статус успешно выдан!</b>\n\n👤 ID: <code>{user_id}</code>\n🏷️ Статус: <b>{new_status}</b> (проверено: {actual_status})\n\nСпасибо за сотрудничество 🤝"
    await message.answer(log_text)
    
    try:
        await bot.send_message(user_id, f"✅ <b>Вам выдан статус: {new_status}</b>\n\nby @teqqines")
    except: pass
    await state.clear()
...
async def main():
    ...
    logger.info("✅ ФИНАЛЬНЫЙ БОТ ЗАПУЩЕН (приоритет исправлен + статусы + промокоды + поиск)")
